...
test('action links are retained while footer and asset links are removed', () => {
  ...
  assert.equal(links.some((item) => item.url.includes('/privacy')), false);
  assert.equal(links.some((item) => item.url.includes('logo.png')), false);
});
